...
#
# Калибровка (КОНСЕРВАТИВНАЯ — базовая линия для органики):
#   - P(LATENT→LATENT)=0.92: нарратив почти всегда остаётся скрытым
#   - P(EMERGING→GROWING)=0.08: прорыв — редкость
#   - P(GROWING→VIRAL)=0.05: вирусная фаза — исключение
#   - Сильный pull-back: EMERGING→LATENT=0.30
#   - Координированные сценарии сдвигают вероятности нелинейно
#     через dynamic_matrix в abm_engine.py
MARKOV_BASE_MATRIX: np.ndarray = np.array([
    # LAT    EMG    GRW    VIR    DEC
    [0.97,  0.02,  0.000, 0.000, 0.01],   # Латентная: ТОЛЬКО в зарождение
    [0.45,  0.42,  0.02,  0.000, 0.11],   # Зарождение: 45% откат, 0% вирусная
    [0.20,  0.35,  0.33,  0.001, 0.119],  # Рост: 0.1% вирусная без координации
    [0.00,  0.02,  0.08,  0.35,  0.55],   # Вирусная
    [0.30,  0.05,  0.01,  0.00,  0.64],   # Затухание
], dtype=np.float64)
...
# ═══════════════════════════════════════════════════════════════════
# 6b. ТЕМАТИЧЕСКИЕ КАТЕГОРИИ МОНИТОРИНГА (Санжар / Саятбек С.)
# ═══════════════════════════════════════════════════════════════════
#
# Чувствительные темы для казахстанского информационного пространства.
# Каждая тема — набор ключевых слов (EN/RU/KZ) для классификации
# статей из GDELT по заголовкам. Классификация — rule-based:
# если заголовок содержит хотя бы одно ключевое слово → тема.

NARRATIVE_TOPICS: Dict[str, Dict[str, Any]] = {
    "ethnic": {
    ...
    "language": {
        "label_ru": "Языковой вопрос",
        "color": "#FFA726",
        "keywords": [
            "language policy", "language law", "kazakh language",
    ...
    "geopolitics": {
        "label_ru": "Геополитическое давление",
        "color": "#5C6BC0",
        "keywords": [
    ...
    "security": {
        "label_ru": "Безопасность и терроризм",
        "color": "#78909C",
        "keywords": [
    ...
    "reputation": {
        "label_ru": "Репутация и инвестиции",
        "color": "#4CAF50",
        "keywords": [
